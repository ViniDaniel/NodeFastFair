...
function Temperos() {
  ...
  useEffect(() => {
    async function fetchTemperos() {
      try {
        const response = await api.get("/produtos/categorias/Temperos");
        const embaralhados = response.data.sort(() => Math.random() - 0.5);
        setProdutos(embaralhados);
      } catch (error) {
        console.error("Erro ao buscar temperos:", error);
      }
    }
  ...
  return (
    <div>
      ...
      <div className={styles.vitrineContainer}>
        <h1 className={styles.pageTitle}>Temperos</h1>
